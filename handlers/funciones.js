    ...
    if (guildid) {
        ...
        if(!setupsdata){
            setupsdata = await new setupSchema({
                guildID: guildid,
                reaccion_roles: [],
                systema_tickets: { canal: "", mensaje: "" },
                sugerencias: "",
                bienvenida: { canal: "", mensaje: "", imagen: "" },
                niveles: { canal: "", mensaje: "" },
                antilinks: false,
                antiDiscord: false,
                miniaturas: false
            });
            await setupsdata.save();
            console.log(`Asegurado: Setups en ${guildid}`.green);
        }
        ...
        if(!serverdata){
            ...
            console.log(`Asegurado: Config en Server de ${guildid}`.green);
        }
    }
    if (userid) {
...
async function paginacion(client, message, texto, titulo = "Paginación", elementos_por_pagina = 5,adorno=false, user=null) {
    ...
    for (let i = 0; i < texto.length; i += dividido) {
        ...
        if(user!== null){embed.setThumbnail(user.displayAvatarURL({ dynamic: true }))}else{embed.setThumbnail(message.guild.iconURL({ dynamic: true }))}
        if(adorno){embed.setTimestamp().setFooter({ text: "~ ElainaNew", iconURL: `${client.user.displayAvatarURL()}` })}
        embeds.push(embed)
    }
        ...
        switch (b?.customId) {
            case "Atrás": {
                ...
                if (paginaActual !== 0) {
                    ...
                    await b?.deferUpdate().catch((e)=>{console.log(e)});
                } else {
    ...
    collector.on("end", () => {
        ...
        embedpaginas.edit({ content: `⏳ ** *El tiempo ha expirado!* **`, embeds: [embeds[paginaActual].setFooter({ text: `Página ${paginaActual + 1} / ${embeds.length}`, iconURL: `${client.user.displayAvatarURL()}` })], components: [embedpaginas.components[0]] }).catch(() => { });
    });
}